...
def carregar_dados_nuvem_raw():
    """Lê e retorna o JSON cru da planilha (sem cache para permitir recarga manual)."""
...
def recalcular_media_casuais(anos_meses):
    """Calcula a média mensal de gastos casuais nos últimos N meses/anos fornecidos."""
    valores = []
...
if "dados_carregados" not in st.session_state:
    ...
    # Migração de renda antiga para o novo formato
...
if ver_projecao:
    ...
    # Coleta meses/anos anteriores para calcular média de gastos casuais (últimos 3 meses disponíveis)
    meses_anteriores = []
    ano_atu = st.session_state.ano_atual
    mes_atu = MESES[st.session_state.mes_atual]
    for i in range(1, 4):
        ano_c = ano_atu
        mes_c = mes_atu - i
        if mes_c <= 0:
    ...
    for i in range(6):
        ...
        proj.append({
            "Mês": f"{list(MESES.keys())[mes_futuro-1]}/{ano_futuro}",
            "Renda": total_renda,
            "Fixos": t_fix,
            "Casuais (média)": media_casuais,
            "Guias": total_guias,
            "Despesa Total": total_despesas,
            "Sobra": sobra_proj
        })
