...
dados_iniciais = {
    ...
    "💳 Itaú": pd.DataFrame({
        ...
        "Ano Início": [2026, 2026, 2026, 2026, 2026],
        "Qtd Parcelas": [12, 12, 12, 12, 12],
        "Valor Parcela (R$)": [4.99, 34.90, 54.80, 19.90, 12.50]
    }),
    "💳 Inter": pd.DataFrame({
        "Descrição": ["Spotify"],
        "Mês Início (1-12)": [5],
        "Ano Início": [2026],
        "Qtd Parcelas": [12],
        "Valor Parcela (R$)": [31.90]
    }),
    "💳 Mercado Pago": pd.DataFrame({
        "Descrição": ["Perfume", "Cafés", "Cama", "Encosto Carro", "Limpador", "Pressca", "Capa e Correia", "Compra s/ nome", "Pedestal e Suporte", "AliExpress", "Mixer", "Tenis"],
        "Mês Início (1-12)": [4, 4, 3, 3, 3, 2, 1, 1, 12, 12, 11, 9],
        "Ano Início": [2026, 2026, 2026, 2026, 2026, 2026, 2026, 2026, 2025, 2025, 2025, 2025],
        "Qtd Parcelas": [4, 2, 4, 7, 4, 4, 6, 5, 6, 6, 8, 11],
        "Valor Parcela (R$)": [106.80, 43.43, 268.23, 20.14, 12.23, 31.49, 21.69, 12.63, 21.33, 24.19, 10.09, 25.66]
    }),
    "💳 Will": pd.DataFrame({
        "Descrição": ["Air BNB", "Mariana", "Passagens"],
        "Mês Início (1-12)": [1, 12, 11],
        "Ano Início": [2026, 2025, 2025],
        "Qtd Parcelas": [6, 6, 10], # Assumindo passagens em 10x
        "Valor Parcela (R$)": [374.52, 47.00, 187.22]
    })
}

# Inicializa as guias com os dados pré-cadastrados ou em branco
...
with st.sidebar:
    ...
    nova_guia = st.text_input("Nome da Guia (ex: Reforma):")
...
for i, guia in enumerate(st.session_state.guias_extras, start=3):
    with abas[i]:
        ...
        st.caption("Altere os valores aqui. O sistema calcula automaticamente em qual mês a parcela vai cair.")
